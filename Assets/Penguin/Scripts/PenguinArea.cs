...
public class PenguinArea : MonoBehaviour
{
    ...
    public void RemoveSpecificBone (GameObject boneObject)
    {
        boneList.Remove(boneObject);
        Destroy(boneObject);
    }
    ...
    public int BoneRemaining
    {
        get { return boneList.Count; }
    }
    ...
    private void RemoveAllFishAndBone()
    {
        if (fishList != null)
        {
    ...
    private void PlaceBin()
    {
        ...
        rigidbody.velocity = Vector3.zero;
        rigidbody.angularVelocity = Vector3.zero;
        trashBin.transform.position = ChooseRandomPosition(transform.position, -45f, 45f, 4f, 9f) + Vector3.up * .5f;
        trashBin.transform.rotation = Quaternion.Euler(-90f, 0f, 90f);

        if (Vector3.Distance(trashBin.transform.position, penguinBaby.transform.position) <= 3)
        {
            PlaceBin();
        }
    }
    ...
    private void SpawnBone(float count)
    {
        for (int i = 0; i < count; i++)
        {
            GameObject boneObject = Instantiate<GameObject>(bonePrefab.gameObject);
            boneObject.transform.position = ChooseRandomPosition(transform.position, 100f, 260f, 2f, 13f) + Vector3.up * .5f;
            boneObject.transform.rotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0f, 360f), 0f);

            boneObject.transform.SetParent(transform);

            boneList.Add(boneObject);
        }
    }
